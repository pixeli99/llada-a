...
class OnlineDataset(Dataset):
    ...
    def __init__(self, 
        ...
        self.jwt_token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiJ3ZW54aW4zQGxpeGlhbmcuY29tIiwiaXNzIjoibHBhaSIsImlhdCI6MTY0NzU3MzkwNywianRpIjoiOWNiNzc0NjQtZGVjYS00NTRhLWIyZTEtYmI4NTA2N2VjM2JhIn0.Krz29HMzS6z762FsxjSUbbNJ0qRchqmtPM3FK2lrXrs"
    ...
    def prepare_raw_data(self, index_or_path):
        """
        准备指定样本的原始数据，接受索引或完整文件路径
        
        Args:
            index_or_path: 数据集索引或直接的npz文件路径
            
        Returns:
            dict: 原始数据字典，包含场景信息
        """
        # 检查输入是索引还是文件路径
        if isinstance(index_or_path, (int, np.integer)):
            info_path = self.get_info_path(index_or_path)
        else:
            # 直接使用提供的文件路径
            info_path = index_or_path
        
        info = self.load_np_file(info_path.strip())
        # 确保即使npz内容中缺少scene_token也能存在
